...
export function parseLightningAddress(address: string): URL | undefined {
  ...
  try {
    return new URL(
      `${scheme}://${domain}/.well-known/lnurlp/${encodeURIComponent(name)}`
    );
  } catch {
    return undefined;
  }
...
export async function requestInvoice(
  callback: string | URL,
  msats: number,
  opts?: { comment?: string; nonce?: string }
): Promise<LnurlCallbackResponse> {
  const cb =
    typeof callback === "string" ? new URL(callback) : new URL(callback.toString());
...
export function verifyInvoiceAmount(pr: string, expectedMsats: number): void {
  ...
  if (parsed.amount !== expectedMsats)
    throw new Error("Invoice amount does not match requested amount");
}

export function verifyInvoiceDescriptionHashIfAvailable(
  pr: string,
  metadataString: string
): void {
  // Best-effort: if parser exposes descriptionHash we compare it, otherwise skip silently
  try {
    const parsed = parseBolt11(pr) as {
      descriptionHash?: string;
      tags?: { h?: string };
      h?: string;
    } | null;
    const dh: string | undefined =
      parsed?.descriptionHash || parsed?.tags?.h || parsed?.h;
    if (!dh) return; // parser doesn't expose; skip
    const hash = bytesToHex(sha256(utf8ToBytes(metadataString)));
    if (dh.toLowerCase() !== hash.toLowerCase())
      throw new Error("Invoice description hash does not match metadata");
  } catch {
...
export async function executeLnurlPayment(options: {
  ...
  const { params, lnAddress, domain } = resolved;
  ...
  const callbackResponse = await requestInvoice(params.callback, amountMsats, {
    comment: comment
      ? comment.slice(0, params.commentAllowed ?? comment.length)
      : undefined,
  });
 ...
 */
export function isBolt11Invoice(str: string): boolean {
